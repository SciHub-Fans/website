...
interface RippleButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  ...
  changeChild?: React.ReactNode;
}

export default function RippleButton({ children, changeChild, ...props }: RippleButtonProps) {
  const buttonRef = useRef<HTMLButtonElement>(null);
  const rippleRef = useRef<HTMLSpanElement>(null);
  const [isHovered, setIsHovered] = useState(false);
  const [openChangeChild, setOpenChangeChild] = useState(false);
  ...
  return (
    <button
      ref={buttonRef}
      className="duration-[300ms] relative flex items-center justify-center overflow-hidden rounded-full bg-white px-6 py-4 rounded-[44px] font-medium text-black transition hover:text-white"
      onMouseEnter={(e) => {
      ...
      {...props}
    >
      <span className="relative z-[2] flex items-center gap-2.5">{openChangeChild ? changeChild : children}</span>
